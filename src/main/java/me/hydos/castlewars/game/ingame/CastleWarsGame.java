...
import me.hydos.castlewars.entity.ProtectThisEntity;
...
public class CastleWarsGame {
    ...
    private static final WorldBorder TEAM_BORDER = new WorldBorder();
    ...
    private ActionResult onUseBlock(ServerPlayerEntity serverPlayerEntity, Hand hand, BlockHitResult blockHitResult) {
        return isIllegal(serverPlayerEntity.getStackInHand(hand));
    }

    private ActionResult isIllegal(ItemStack item) {
    ...
    private ActionResult isIllegal(Block block) {
    ...
    private void tick() {
        ...
        if (ticks == 20 * 300) { // 5 minutes
            ...
            for (ServerPlayerEntity player : PlayerManager.getInstance().participants.keySet()) {
                ...
                player.networkHandler.sendPacket(new TitleS2CPacket(TitleS2CPacket.Action.SUBTITLE, new LiteralText("Teams Thing idk").formatted(Formatting.YELLOW, Formatting.BOLD)));
                player.networkHandler.sendPacket(new WorldBorderS2CPacket(TEAM_BORDER, WorldBorderS2CPacket.Type.SET_SIZE));
    ...
    static {
        TEAM_BORDER.setCenter(0, 0);
        TEAM_BORDER.setSize(100000d);
    }
}
