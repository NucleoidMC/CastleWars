...
public class CastleWarsMap {
    ...
    public void close(CastleWarsGame game) {
        for (ServerPlayerEntity player : PlayerManager.getInstance().participants.keySet()) {
            player.networkHandler.sendPacket(new BossBarS2CPacket(BossBarS2CPacket.Type.REMOVE, blueTeam));
            player.networkHandler.sendPacket(new BossBarS2CPacket(BossBarS2CPacket.Type.REMOVE, redTeam));
        }
    }
}
